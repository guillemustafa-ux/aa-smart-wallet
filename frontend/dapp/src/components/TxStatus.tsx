import type { TxStatus as TxStatusType } from "../hooks/useSmartAccount";
...
export function TxStatus({ status, txHash, txError, chainId, onDismiss }: Props) {
  ...
  return (
    <div className={`mt-4 p-3 rounded-xl text-sm flex items-start gap-2 border ${styles[key]}`}>
      {status === "pending" && (
        <svg className="size-4 animate-spin shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none" aria-hidden="true">
      ...
      <div className="flex-1 min-w-0 break-words">
        {status === "pending" && "Firmá en la wallet… (no pagás gas) y esperá la inclusión"}
        {status === "confirmed" && (
          <>
            ¡Listo, gasless!{" "}
            {txHash && net && (
